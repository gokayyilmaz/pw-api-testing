...
test("create, update, delete article", async ({ request }) => {
  ...
  expect(updateArticleResponseBody.article.title).toBe(`testTitle${randomNumber} updated`)
  ...
  const getArticlesResponseBody = await getArticlesResponse.json()
  expect(getArticlesResponse.status()).toBe(200);
  expect(getArticlesResponseBody.articles[0].title).toBe(`testTitle${randomNumber} updated`)
